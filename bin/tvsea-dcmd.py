...
def matchDownloadFile(f, key_words, epid, epsode_id_type, resolution, release_group):
    ...
    for kw in key_words:
        if fu.find(kw.upper()) < 0:
            #logger.debug("Fail keyword matching: {} {}".format(kw.upper(), fu.find(kw.upper())))
            return False
...
def distByMv(q, d, t):
    ...
    if os.path.isfile(source):
        ...
        logger.warn("Source file is not exist: {}".format(source))
...
def distByRepac(q, d, t):
    ...
    os.system(cstr)
    logger.info("Complete repackaging to plex lib: {}".format(target))
...
def distByTrans(q, d, t):
    ...
    logger.info("Command: {}".format(cstr))
    os.system(cstr)
    logger.info("Complete transcoding to plex lib: {}".format(target))
