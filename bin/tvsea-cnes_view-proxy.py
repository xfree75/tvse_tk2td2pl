...
def getKimKtvList(tvGenreName):
    ...
    kimBaseUrl = "https://torrentview18.net:443/bbs/board.php?bo_table=" + tvGenreName
...
def checkRecentUpdate():
    ...
    for file in os.listdir(os.path.join(rspath, CONST.feedlib_path_name)):
        if file.endswith(".json") and file.startswith("view_"):
...
def updatekimfeed():
    ...
    saveJsonArticle(getKimKtvList("drama"), "view_" + CONST.dramafeedlib_name)
    saveJsonArticle(getKimKtvList("enter"), "view_" + CONST.entertainmentfeedlib_name)
    saveJsonArticle(getKimKtvList("tv"),    "view_" + CONST.documentaryfeedlib_name)
...
def findNewKimEpsoides():
    ...
    feedlibs = []
    for feedfile in glob.glob(os.path.join(rspath, CONST.feedlib_path_name) + '/view_*.json'):
        ff = open(feedfile, 'r')
