...
def listhtml2obj(htmlstring):
    ...
    soup_lilist = soup_board_list[0].find_all('li')
    #logger.debug("html2: {}".format(soup_lilist[0].prettify(formatter="html")))
    #sys.exit()
    ...
    listidx = 0
    for soup_li in soup_lilist:
        listidx = listidx + 1
        #logger.debug("li html: {}".format(soup_li.prettify(formatter="html")))
        ...
        #ahrefs = soup_td2.div.find_all('a', recursive=False)
        #ahref = soup_td2.div.find_all('a', recursive=False)[len(ahrefs)-1]
        #logger.debug("ahrefs: {}".format(ahrefs))
        #logger.debug("ahrefs length: {}".format(len(ahrefs)))
        #detailurl = ahref['href']
        #logger.debug("detail url: {}".format(detailurl))
        ...
        '''날짜'''
        _v_date = ""
        _vv_date = soup_li('div', {'class':'wr-date hidden-xs'})[0].find_all('span', recursive=False)
        if len(_vv_date) == 0:
            _v_date = soup_li('div', {'class':'wr-date hidden-xs'})[0].string.strip()
        else:
            _v_date = soup_li('div', {'class':'wr-date hidden-xs'})[0].find_all('span', recursive=False)[0].string.strip()
        
        logger.debug("______________________________________________________")
        logger.debug("number: {}".format(_v_number))
        logger.debug("title : {}".format(_v_title))
        logger.debug("url   : {}".format(_v_detailurl))
        logger.debug("size  : {}".format(_v_filesize))
        logger.debug("date  : {}".format(_v_date))
        
        # object 생성.
        torrcontent = {}
        torrcontent['num']       = _v_number
        torrcontent['title']     = _v_title
        torrcontent['url']       = _v_detailurl
        torrcontent['size']      = _v_filesize
        torrcontent['date']      = _v_date
        torrcontent['publisher'] = "al"
...
def getKtvList(tvGenreName):
    ...
    baseUrl = "https://torrental2.com/bbs/board.php?bo_table=" + tvGenreName
    ...
    for pagenum in range(1, pageCountForFeed + 1):
        ...
        logger.info("Download status: {} / {}".format(r.status_code, baseUrl + pagePath))
        if r.status_code == 200:
            data = r.content
            #logger.info("Data : {}".format(data))
            contentlist = contentlist + listhtml2obj(data.decode())
...
def checkRecentUpdate():
    ...
    for file in os.listdir(os.path.join(rspath, CONST.feedlib_path_name)):
        if file.endswith(".json") and file.startswith("al_"):
...
def updatemapfeed():
    ...
    saveJsonArticle(getKtvList("kordrama"), "al_" + CONST.dramafeedlib_name)
    saveJsonArticle(getKtvList("korent"),   "al_" + CONST.entertainmentfeedlib_name)
    saveJsonArticle(getKtvList("koraffair"),   "al_" + CONST.documentaryfeedlib_name)
...
def downloadFromAlMagnet(tpe, title_keywords):
    ...
    logger.debug("parser netloc result:{}".format(pr.netloc))
    ...
    if r1.status == 200:
        ...
        ahrefs = soup.find_all('a', recursive=True)
        #logger.debug("eleSection: {}".format(ahrefs))
        magnet_string = ""
        for ahref in ahrefs:
            #logger.debug("current a element: {}".format(ahref))
            if ahref.has_attr('href'):
                curl = ahref['href']
                if curl.startswith("magnet:?xt=urn:btih:"):
                    magnet_string = curl
                    logger.info("found the magnet: {}".format(curl)) 
...
def discoveryAndDownload(ed, leid, feedlibs):
    ...
    for k in match3feedDic.keys():
        ...
        if te:
            ...
            #downloadFromMapMagnet(te, title_keywords)
            downloadFromAlMagnet(te, title_keywords)
...
def findNewEpsoides():
    ...
    feedlibs = []
    for feedfile in glob.glob(os.path.join(rspath, CONST.feedlib_path_name) + '/al_*.json'):
        ff = open(feedfile, 'r')
