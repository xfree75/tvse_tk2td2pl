...
def kimlisthtml2obj(htmlstring):
    soup = BeautifulSoup(htmlstring, "lxml")
    soup_board_list = soup('div', {'class':'bo_tit'})
    ...
    listidx = 0
    for soup_tr in soup_board_list:
        listidx = listidx + 1
        #logger.debug("tr html: {}".format(soup_tr.prettify(formatter="html")))
        
        try:
            soup_a = soup_tr.find_all('a', recursive=False)[0]
            #logger.debug("a: {}".format(soup_a.prettify(formatter="html")))
            #continue
            logger.debug("soup_td_subject     : {}".format(soup_a.text.strip()))
            logger.debug("soup_td_subject_href: {}".format(soup_a['href']))
            
            # object 생성.
            torrcontent = {}
            torrcontent['num']       = ""
            torrcontent['title']     = soup_a.text.strip()
            torrcontent['url']       = "https://" + base_dn + ":443" + soup_a['href']
            torrcontent['date']      = ""
            #torrcontent['size']      = soup_td_size.string.strip()
            torrcontent['publisher'] = "bom"
            torrcontentlist.append(torrcontent)
...
def getKimKtvList(tvGenreName):
    ...
    kimBaseUrl = "https://" + base_dn + ":443/list/" + tvGenreName
    ...
    for pagenum in range(1, pageCountForFeed + 1):
        ...
        kimPagePath = "/torrent_" + str(pagenum) + ".html"
...
def checkRecentUpdate():
    ...
    for file in os.listdir(os.path.join(rspath, CONST.feedlib_path_name)):
        if file.endswith(".json") and file.startswith("bom_"):
...
def updatekimfeed():
    ...
    saveJsonArticle(getKimKtvList("torrent_korea_drama"),   "bom_" + CONST.dramafeedlib_name)
    saveJsonArticle(getKimKtvList("torrent_variety"), "bom_" + CONST.entertainmentfeedlib_name)
    saveJsonArticle(getKimKtvList("torrent_docu"),    "bom_" + CONST.documentaryfeedlib_name)
...
def downloadFromKimMagnet(tpe, title_keywords):
    ...
    if r1.status == 200:
        ...
        soup_bo_v_img_list = soup.find_all("li")
        #logger.debug("Li elements: {}".format(soup_bo_v_img_list))
        
        for soup_bo_v_img in soup_bo_v_img_list:

            try:
                #soup_bo_v_img.b.decompose()
                if soup_bo_v_img.find('b') != None:
                    soup_bo_v_img.find('b').decompose()

                li_val_str = soup_bo_v_img.text.strip()[2:]
                #logger.debug("Li emelent: {}".format(li_val_str))
                
                if li_val_str.startswith('magnet:?'):
                    logger.info("Start adding magnet: {}({}) s{} e{} : {}".format(ed["series_name"], ed["release_year"], ed["season_number"], tpe["epid"], tpe["title"]))
                    logger.debug("magnet: {}".format(li_val_str))
                    cmdstr = "transmission-remote --auth=" + ctru + ":" + ctrp + " -a \"" + li_val_str + "\""
...
def findNewKimEpsoides():
    ...
    feedlibs = []
    for feedfile in glob.glob(os.path.join(rspath, CONST.feedlib_path_name) + '/bom_*.json'):
        ff = open(feedfile, 'r')
