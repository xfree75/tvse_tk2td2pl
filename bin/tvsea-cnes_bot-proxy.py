...
def loadConfig():
    ...
    global base_dn
    base_dn = config["base_dn_bot"]


def kimlisthtml2obj(htmlstring):
    soup = BeautifulSoup(htmlstring, "lxml")
    soup_board_list = soup('ul', {'class':'customer_table'})
    #logger.debug("html: {}".format(soup_board_list[0].prettify(formatter="html")))
    soup_trlist = soup_board_list[0].find_all('ul', {'class':'ufl'})
    ...
    for soup_tr in soup_trlist:
        ...
        try:
            ...
            torrcontent['url']       = "https://" + base_dn + ":443" + url
            torrcontent['date']      = ""
            #torrcontent['size']      = soup_td_size.string.strip()
            torrcontent['publisher'] = "bot"
...
def getKimKtvList(tvGenreName):
    ...
    kimBaseUrl = "https://" + base_dn + ":443/topic/index?category1=" + tvGenreName
...
def checkRecentUpdate():
    ...
    for file in os.listdir(os.path.join(rspath, CONST.feedlib_path_name)):
        if file.endswith(".json") and file.startswith("bot_"):
...
def updatekimfeed():
    ...
    saveJsonArticle(getKimKtvList("2&category2=13"), "bot_" + CONST.dramafeedlib_name)
    saveJsonArticle(getKimKtvList("4&category2=16"), "bot_" + CONST.entertainmentfeedlib_name)
    saveJsonArticle(getKimKtvList("4&category2=17"),    "bot_" + CONST.documentaryfeedlib_name)
...
def findNewKimEpsoides():
    ...
    feedlibs = []
    for feedfile in glob.glob(os.path.join(rspath, CONST.feedlib_path_name) + '/bot_*.json'):
        ff = open(feedfile, 'r')
