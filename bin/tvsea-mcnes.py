...
def listhtml2obj(htmlstring):
    ...
    soup_board_list = soup('div', {'class':'tbl_head01 tbl_wrap'})
    #logger.debug("html1: {}".format(soup_board_list[0].prettify(formatter="html")))
    soup_tbody = soup_board_list[0].find_all('tbody')
    #logger.debug("html2: {}".format(soup_tbody[0].prettify(formatter="html")))
    soup_trlist = soup_tbody[0].find_all('tr')
    #logger.debug("html3: {}".format(soup_trlist[0].prettify(formatter="html")))
    #sys.exit()
...
def getMapKtvList(tvGenreName):
    ...
    baseUrl = "https://www.torrentmap.com/bbs/board.php?bo_table=" + tvGenreName
...
def checkRecentUpdate():
    ...
    for file in os.listdir(os.path.join(rspath, CONST.feedlib_path_name)):
        if file.endswith(".json") and file.startswith("map_"):
...
def updatemapfeed():
    ...
    saveJsonArticle(getMapKtvList("kr_drama"), "map_" + CONST.dramafeedlib_name)
    saveJsonArticle(getMapKtvList("kr_ent"),   "map_" + CONST.entertainmentfeedlib_name)
    saveJsonArticle(getMapKtvList("kr_daq"),   "map_" + CONST.documentaryfeedlib_name)
...
def downloadFromMapMagnet(tpe, title_keywords):
...
def findNewEpsoides():
    ...
    feedlibs = []
    for feedfile in glob.glob(os.path.join(rspath, CONST.feedlib_path_name) + '/map_*.json'):
        ff = open(feedfile, 'r')
